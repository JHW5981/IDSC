...
def calcu_twice_similar_grad_D(coefficients: np.ndarray) -> np.ndarray:
    """
    Compute the twice gradient of 1/2||X-DA||^2 with respect to D (atoms).
    twice_grad_D = A A^T
    ...
    Returns:
        np.ndarray: The twice gradient matrix with respect to D.

    Examples:
        >>> twice_similar_grad_D = calcu_twice_similar_grad_D(patches, atoms, coefficients)
    """
    
    twice_grad_D = np.matmul(coefficients, coefficients.T)
...
def calcu_huber_grad_A(coefficients: np.ndarray,
    ...
    grad_A = np.zeros_like(coefficients)
    
    # if |a| <= M
    masks = np.where(np.abs(coefficients) <= huber_M)
    grad_A[masks] = 2 * coefficients[masks]

    # if |a| > M
    masks = np.where(np.abs(coefficients) > huber_M)
    grad_A[masks] = 2 * huber_M * np.sign(coefficients[masks])
...
def backtracking_ls_D(descent_direction: np.ndarray,
    ...
    function_value = minimizing_function(patches, atoms, coefficients, penalty_type, huber_M, penalty_weight)
    grad_D = calcu_similar_grad_D(patches, atoms, coefficients)
...
def minimizing_function(patches: np.ndarray,
    ...
    if penalty_type == 'l1':
        ...
        huber = np.zeros_like(coefficients)
    
        # if |a| <= M
        masks = np.where(np.abs(coefficients) <= huber_M)
        huber[masks] = coefficients[masks] * coefficients[masks]

        # if |a| > M
        masks = np.where(np.abs(coefficients) > huber_M)
        huber[masks] = huber_M * (np.abs(coefficients[masks]) * 2 - huber_M)
