...
def get_arguments():
    ...
    # experiment
    parser.add_argument('--data_path', default="../images/gray/barbara_512.png", help='path to the data')
    parser.add_argument('--output_directory', default='outputs', help='directory to save outputs')
    parser.add_argument("--original_img_shape", default=(512, 512), help="original image shape")
    ...
    # train
    parser.add_argument("--max_iters", type=int, default=200, help="the number of iterations")
...
def main():
    ...
    # read image
    image_np = read_image(Path(args.data_path), args.img_shape)  # (512, 512)

    # extract patches from image
    patches = extrat_patches(image_np, args.patch_size, args.patch_gap)  # (8, 8, patch_num)

    # random init atoms and coefficients 
    atoms = np.random.randn(*args.patch_size, args.num_atoms)   # (8, 8, 128)
    ...
    atoms = atoms.reshape(-1, atoms.shape[-1])   # (64, 128)
    ...
    save_atom_image(atoms.reshape(*args.patch_size, atoms.shape[-1]), os.path.join(output_dir, 'atoms.png'))
    np.save(os.path.join(output_dir, 'coefficients.npy'), coefficients)
    
    # reconstruct image from dictionary learning
    reconstruct_patches = np.matmul(atoms, coefficients)  # (64, patch_num)
    reconstruct_patches = reconstruct_patches * patches_std + patches_mean
    reconstruct_patches = reconstruct_patches.reshape(*args.patch_size, reconstruct_patches.shape[-1]) # (8, 8, patch_num)
    reconstruct_images = combine_patches(reconstruct_patches, args.img_shape, args.patch_gap)  # (512, 512)
