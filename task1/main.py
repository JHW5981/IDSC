...
def get_arguments():
    ...
    parser.add_argument('--huber_M', type=float, default=1.0, help='M for Huber penalty function')
    parser.add_argument("--penalty_weight", type=float, default=10.0, help="the loss weight for sparse penalty term")

    # train
    parser.add_argument("--max_iters", type=int, default=2000, help="the number of iterations")
...
def main():
    ...
    patches = extrat_patches(image_np, args.patch_size)  # (8, 8, 3, 4096)

    # random init atoms and coefficients 
    atoms = np.random.rand(*args.patch_size, 3, args.num_atoms)   # (8, 8, 3, 128)
    coefficients = np.random.randn(args.num_atoms, patches.shape[-1])   # (128, 4096)
    
    # dictionary learning
    patches = patches.reshape(-1, patches.shape[-1])  # (192, 4096)
    ...
    for iter in range(1, args.max_iters):
        ...
        # update atoms
        atoms_grad = calcu_similar_grad_D(patches, atoms, coefficients)
        if args.descent_type_D == 'gradient':
            descent_direction_D = - atoms_grad
        elif args.descent_type_D == 'newton':
            descent_direction_D = - np.matmul(atoms_grad, np.linalg.pinv(calcu_twice_similar_grad_D(coefficients)))
        if args.step_size_type == 'bls':
    ...
    reconstruct_patches = np.matmul(atoms, coefficients)  # (192, 4096)
    reconstruct_patches = reconstruct_patches.reshape(*args.patch_size, 3, reconstruct_patches.shape[-1]) # (8, 8, 3, 4096)
    reconstruct_images = combine_patches(reconstruct_patches, args.img_shape)  # (512, 512, 3)
