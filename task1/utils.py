...
def combine_patches(patches: np.ndarray, 
    ...
    patch_height, patch_width, _ = patches.shape

    combined_image = np.zeros([height, width])
    count_overlap = np.zeros([height, width])
...
def save_atom_image(atom_np: np.ndarray,
    ...
    """
    patch_height, patch_width, atom_num = atom_np.shape
    ...
    for i in range(atom_num):
        ...
        plt.imshow(atom_np[:, :, i], cmap = plt.cm.gray_r, interpolation='nearest')
        plt.xticks(())
        plt.yticks(())
